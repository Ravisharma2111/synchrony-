...
import rejectIcon from '../../assets/icon/cry.svg';
  ...
  return (
    <Card className={styles.swapCard} style={containerStyle}>
      ...
      <div style={contentStyle}>
        ...
        <InfoLabel />
        <SwapButton transactionStatus={transactionStatus} setTransactionstatus={setTransactionstatus}/>
      </div>
    </Card>
  );
}
...
export function SwapButton({transactionStatus,setTransactionstatus}) {
  const styles = useStyles();
  const {
    fromMint,
    toMint,
  ...
  return (
    ...
    <Dialog
           ...
           <MuiDialogContent className={classes.swapModel}>
            ...
            <Typography>{fromAmount}</Typography>
                </div>
            <div className={classes.innerethcalc}><Typography>{ (toTokenInfo && fromTokenInfo) && fromTokenInfo.symbol }</Typography></div>
             </div>
            ...
            <Typography>{toAmount}</Typography>
                </div>
            <div className={classes.innerethcalc}><Typography>{ (toTokenInfo && fromTokenInfo) && toTokenInfo.symbol }</Typography></div>
             </div>
                <Button className={classes.swapConfirmbtn} onClick={sendSwapTransaction} >Confirm Swap</Button>
                 </> 
                :
                <>
                {transactionStatus === 3 ?
                 <> <div className={classes.loadingSwap}>
                  ...
                  <div className={classes.lastCalcswap}>
                    <Typography>{fromAmount} { (toTokenInfo && fromTokenInfo) && fromTokenInfo.symbol }</Typography>
                    <img src={rightIcon} alt=""/>
                    <Typography>{toAmount} { (toTokenInfo && fromTokenInfo) && toTokenInfo.symbol }</Typography>
                 </div>
                  </>
                  :
                <div className={classes.swapLaststeap}>
                 <img src={transactionStatus === 2 ? GcheckIcon : rejectIcon  } alt="" />
                 <Typography>Transaction {transactionStatus === 1 ? <> Rejected </> : <> Confirmed </> }</Typography>
                 <div className={classes.lastCalcswap}>
                    <Typography>{fromAmount} { (toTokenInfo && fromTokenInfo) && fromTokenInfo.symbol }</Typography>
                    <img src={rightIcon} alt=""/>
                    <Typography>{toAmount} { (toTokenInfo && fromTokenInfo) && toTokenInfo.symbol }</Typography>
    ...
    <Button
      ...
      onClick={()=>{setOpen(true);setTransactionstatus(0);}}
      disabled={!canSwap}
    >
      Swap
    </Button>
