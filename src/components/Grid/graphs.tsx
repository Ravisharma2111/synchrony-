            ...
            }
            {
                ...
                <>
                    {props.data.datasets.length === 0 ?
                        <div className="nodata"><img src={noDataIcon} alt="" /><p>No Data</p></div>
                        :
                        <> {(props.hideOption !== true) && <div className={classes.pullRight}>
                            <List className={classes.graphBtn}>

                                {props.data.customLables.map((key: any, index: any) => (
                                    <ListItem className={graphData[props.data.activeLabel] === key ? 'active' : ''} key={index} onClick={() => { setChartDatakey(key) }}>
                                        {key}
                                    </ListItem>
                                ))}
