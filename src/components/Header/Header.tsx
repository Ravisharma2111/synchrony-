...
import Ethereum from '../../assets/pool/ethereum.svg';
...
const Header = ({ history }: RouteComponentProps) => {
  ...
  const [walletMoreAnchorEl, setWalletMoreAnchorEl] = React.useState<null | HTMLElement>(null);
  const [openModal, setOpenModal] = useState(false);
  const isMenuOpen = Boolean(anchorEl);
  const isMobileMenuOpen = Boolean(mobileMoreAnchorEl);
  const isCartMenuOpen = Boolean(cartMoreAnchorEl);
  const isWalletMenuOpen = Boolean(walletMoreAnchorEl);
  ...
  const [mataOpen, setMetaOpen] = useState(false);
  const handleMetaOpen = () => { setMetaOpen(true); setWalletMoreAnchorEl(null) };
  const handleMetaClose = () => setMetaOpen(false);
  ...
  const handleWalletMenuOpen = (event: React.MouseEvent<HTMLElement>) => {
    setWalletMoreAnchorEl(event.currentTarget);
    setAnchorElPopper(null)
  };

  const handleWalletMenuClose = () => {
    setWalletMoreAnchorEl(null);
  }
    ...
    >
      ...
      <MenuItem onClick={handleProfileMenuOpen}>
        <NavLink className="navbar-item" to="/staking">
          <AssistantDirectionIcon /> Staking
        </NavLink>
      </MenuItem>
      {/* <MenuItem> */}
      {/* <NavLink className="navbar-item" to="/staking">
  ...
  const walletItemRender = (
    <Menu
      ...
      className={classes.cartItemData1}
    >
      <span className={classes.headerCart}>My Wallets</span>
      <MenuItem className={classes.cartMenu} onClick={() => setWalletMoreAnchorEl(null)}>
        <LoginButton /> {isWalletMenuOpen}
      </MenuItem>
      {metaMaskWallet === "" ?
        <MenuItem className={classes.cartMenu} onClick={handleMetaOpen}>
          <img src={Ethereum} alt="" className='walletmenu' /> <span> Ethereum</span> {isWalletMenuOpen}
        </MenuItem>
        :
        <MenuItem className={classes.cartMenu}>
        ...
        <Toolbar>
          <div className={classes.logoIcon}>
            ...
            <div className={classes.chipIcon}>
              <Chip className={classes.chipinner} size="small" label="Beta" />
            </div>
          </div>
            ...
            >
              ...
              {/* <ListItem className={classes.menuactive}>
               <NavLink className="navbar-item" to="/leaderboard">
                   <StoreMallDirectoryIcon /> Leaderboard 
               </NavLink>
            </ListItem> */}
          ...
          <div className={classes.sectionDesktop}>
            <div className={classes.rigthIconsHeader}>
              <span className={classes.mianHeaderIcon} onClick={handleWalletMenuOpen}>
